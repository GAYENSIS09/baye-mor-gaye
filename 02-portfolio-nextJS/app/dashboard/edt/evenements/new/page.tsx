...
import { useCreateEvenement } from '@/hooks/mutations';
import { LoadingScreen } from '@/components/LoadingScreen';

function NewEvenementForm() {
  ...
  useEffect(() => {
    if (!authLoading && utilisateur) {
      const edtId = searchParams.get('edt_id');
      if (edtId) setEmploiDuTempsId(edtId);
    }
  }, [authLoading, utilisateur, searchParams]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      console.error('Erreur creation evenement');
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold mb-6 text-off-white">Nouvel evenement</h1>
